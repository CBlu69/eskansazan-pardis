...
function bindEvents() {
    ...
    document.getElementById("add-project")?.addEventListener("click", async () => {
        ...
        let error;
        if (editingProjectId) {
            ({ error } = await client.from("projects").update({
                name, supervisor: pSupervisor.value, progress: pProgress.value,
                buildStatus: pBuildStatus.value, adjustment: pAdjustment.value,
            ...
            ({ error } = await client.from("projects").insert([{
                name, supervisor: pSupervisor.value, progress: pProgress.value,
                buildStatus: pBuildStatus.value, adjustment: pAdjustment.value,
                description: pDescription.value, owner_id: currentUser.id
            }]));
        }
        if (error) return showToast(error.message, 'error');
        pName.value = ""; pSupervisor.value = ""; pProgress.value = "";
        pBuildStatus.value = ""; pAdjustment.value = ""; pDescription.value = "";
        document.getElementById("project-modal").style.display = "none";
        await loadProjects();
    ...
    document.getElementById("add-mission")?.addEventListener("click", async () => {
        ...
        let error;
        if (editingMissionId) {
            ({ error } = await client.from("missions").update({
                name, manager: mManager.value, status: mStatus.value
            }).eq("id", editingMissionId));
            editingMissionId = null;
            document.getElementById("add-mission").textContent = "ثبت";
        } else {
            ({ error } = await client.from("missions").insert([{
                name, manager: mManager.value, status: mStatus.value, owner_id: currentUser.id
            }]));
        }
        if (error) return showToast(error.message, 'error');
    ...
    document.getElementById("add-finance")?.addEventListener("click", async () => {
        ...
        if (file) {
            ...
            const { data } = client.storage.from("finance-files").getPublicUrl(fileName);
            fileUrl = data.publicUrl;
        }
        const { error } = await client.from("financial_requests").insert([{
            title, amount: Number(amount), description, file_url: fileUrl,
            owner_id: currentUser.id, status: "pending", payment_status: "unpaid"
        }]);
        if (error) return showToast(error.message, 'error');
        document.getElementById("finance-modal").style.display = "none";
    ...
    document.getElementById("cancel-delete")?.addEventListener("click", () => {
        ...
        window._deleteType = null;
    });
    document.getElementById("confirm-delete")?.addEventListener("click", async () => {
        const id = window._deleteId;
        const type = window._deleteType;
        document.getElementById("delete-modal").style.display = "none";
        window._deleteId = null;
        window._deleteType = null;
        if (type === "project") {
            ...
            await 
                client.from("financial_requests").delete().eq("id", id);
            await loadFinance();
    ...
    document.getElementById("reject-confirm-btn")?.addEventListener("click", async () => {
        ...
        const { error } = await client.from("financial_requests").delete().eq("id", rejectFinanceId);
        if (error) return showToast(error.message, 'error');
        document.getElementById("reject-modal").style.display = "none";
        rejectFinanceId = null;
        await loadFinance();
...
async function loadProjects() {
    ...
    const { data, error } = await client.from("projects").select("*").eq("owner_id", currentUser.id).order("created_at", { ascending: false });
...
function renderProjects() {
    ...
    if (searchTerm) {
        filtered = allProjects.filter(p => p.name?.toLowerCase().includes(searchTerm) || p.supervisor?.toLowerCase().includes(searchTerm));
    }
    ...
    renderPagination("projects-pagination", projectPage, total, (page) => { projectPage = page; renderProjects(); });
    update();
}
...
async function loadMissions() {
    if (!currentUser) return;
    const { data } = await client.from("missions").select("*").eq("owner_id", currentUser.id).order("created_at", { ascending: false });
    allMissions = data || [];
    missionPage = 1;
    renderMissions();
}
...
function renderMissions() {
    ...
    if (searchTerm) {
        filtered = allMissions.filter(m => m.name?.toLowerCase().includes(searchTerm) || m.manager?.toLowerCase().includes(searchTerm));
    }
    const total = filtered.length;
    ...
    renderPagination("missions-pagination", missionPage, total, (page) => { missionPage = page; renderMissions(); });
    update();
}
...
function renderStaff() {
    ...
    if (searchTerm) {
        filtered = defaultStaff.filter(s => s.name?.toLowerCase().includes(searchTerm) || s.lastname?.toLowerCase().includes(searchTerm) || s.phone?.includes(searchTerm) || s.nationalId?.includes(searchTerm));
    }
...
async function loadFinance() {
    if (!currentUser) return;
    const { data } = await client.from("financial_requests").select("*").order("id", { ascending: false });
    allFinance = data || [];
    financePage = 1;
    renderFinance();
}
...
function renderFinance() {
    ...
    if (searchTerm) {
        filtered = filtered.filter(f => f.title?.toLowerCase().includes(searchTerm) || f.description?.toLowerCase().includes(searchTerm));
    }
    ...
    pageItems.forEach(f => {
        ...
        if (!f.status || f.status === "pending") {
            if (userRole === "admin" || userRole === "manager") {
                actions += `<button class="glass-btn" onclick="approveFinance('${f.id}')">✔ تایید</button>
                <button class="glass-btn danger" onclick="openRejectModal('${f.id}')">✖ رد</button>`;
            }
        }
        if (f.status === "approved" && f.payment_status !== "paid") {
            if (userRole === "admin" || userRole === "finance") {
                actions += `<button class="glass-btn success" onclick="confirmPayment('${f.id}')">💳 تایید پرداخت</button>`;
            }
        }
        if (f.payment_status === "paid" && userRole === "admin") {
            actions += `<button class="glass-btn danger" onclick="deleteFinance('${f.id}')">🗑 حذف</button>`;
        }
        list.innerHTML += `
...
async function loadAllUsers() {
    ...
    if (error) { console.warn("loadAllUsers error:", error.message); return; }
...
window.updateUserRole = async function (userId) {
    ...
    if (userId === currentUser.id) {
        ...
        showUserInfo();
        const adminBtn = document.getElementById("admin-nav-btn");
        if (adminBtn) adminBtn.style.display = newRole === "admin" ? "block" : "none";
    }
...
function renderZonkens() {
    ...
    if (searchTerm) {
        filtered = allZonkens.filter(z => 
            ...
            z.number?.toLowerCase().includes(searchTerm) ||
